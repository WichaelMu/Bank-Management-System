...
namespace BankManagementSystem.Core
{
	public static class AccountParser
	{
		...
		public static bool ReadLogins(out Dictionary<string, string> OutCredentials)
		{
			OutCredentials = new Dictionary<string, string>();
			if (FileSystem.ReadFromFile(FileSystem.kDirectory, FileSystem.kFileName, out List<string> LoginData))
			{
				...
				foreach (string Credential in LoginData)
				{
					...
					if (OutCredentials.ContainsKey(UsernameAndPassword[0]))
					...
					if (UsernameAndPassword.Length == 2)
#endif // WITH_ERROR_CHECKS
						OutCredentials.Add(UsernameAndPassword[0], UsernameAndPassword[1]);
				}
